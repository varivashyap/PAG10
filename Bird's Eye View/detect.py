...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    # If xavg, yavg are PyTorch tensors, move them to CPU and convert to numpy
                    if isinstance(xavg, torch.Tensor):
                        xavg = xavg.cpu().numpy()
                    if isinstance(yavg, torch.Tensor):
                        yavg = yavg.cpu().numpy()
    
                    # Perform perspective transformation
                    field_coord = cv2.perspectiveTransform(
                        np.float32(np.array([[[xavg, yavg]]])),
                        transformation_matrix
                    )[0][0]
                    # Ensure field_coord is a numpy array and extract the x and y coordinates
                    x, y = int(field_coord[0]), int(field_coord[1])

                    # Draw the circle on the image at the specified coordinates
    ...
    if dataset.mode != 'image':
        ...
        height, width, _ = out_video[0].shape  # Assuming all frames have the same shape
        fps = 30  # Adjust as per your video frame rate
        ...
        for frame in out_video:
            video_writer.write(frame.astype(np.uint8))  # Convert frame to uint8 if needed

        video_writer.release()
        LOGGER.info("Video saved as 'video.mp4'")
